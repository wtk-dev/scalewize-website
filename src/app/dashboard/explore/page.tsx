              ...
              <div className="mt-6">
                {solution.status === 'available' ? (
                  ...
                  <button
                    onClick={() => handleAddToTeam(solution.id)}
                    disabled={selectedSolution === solution.id}
                    className="w-full bg-green-600 text-white hover:bg-green-700 px-4 py-2 rounded-md text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                  >
                    {selectedSolution === solution.id ? (
                      <>
