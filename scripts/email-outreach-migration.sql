...
CREATE TABLE IF NOT EXISTS public.outreach_campaigns (
  ...
  business_hours_start integer DEFAULT 9 CHECK (business_hours_start >= 0 AND business_hours_start <= 23),
  business_hours_end integer DEFAULT 17 CHECK (business_hours_end >= 0 AND business_hours_end <= 23),
  send_minute_blocks integer[] DEFAULT '{0,15,30,45}',
...
CREATE TABLE IF NOT EXISTS public.outreach_daily_stats (
  ...
  CONSTRAINT outreach_daily_stats_unique_date UNIQUE (organization_id, campaign_id, date_et)
);

-- 4. Create outreach_messages table for tracking email sends
...
CREATE INDEX IF NOT EXISTS idx_outreach_messages_campaign ON public.outreach_messages(campaign_id, sent_at);

-- 6. Create function to get daily stats for outreach workflow
CREATE OR REPLACE FUNCTION get_outreach_daily_stats(org_id uuid, campaign_id uuid DEFAULT NULL)
RETURNS json AS $$
DECLARE
  result json;
...
BEGIN
  SELECT json_build_object(
    'date_et', CURRENT_DATE AT TIME ZONE tz,
    'or1_count', COALESCE((
      SELECT or1_count FROM public.outreach_daily_stats
      WHERE organization_id = org_id
      AND (campaign_id IS NULL OR campaign_id = get_outreach_daily_stats.campaign_id)
      AND date_et = CURRENT_DATE AT TIME ZONE tz
      ORDER BY created_at DESC LIMIT 1
    ), 0),
    'or2_count', COALESCE((
      SELECT or2_count FROM public.outreach_daily_stats
      WHERE organization_id = org_id
      AND (campaign_id IS NULL OR campaign_id = get_outreach_daily_stats.campaign_id)
      AND date_et = CURRENT_DATE AT TIME ZONE tz
      ORDER BY created_at DESC LIMIT 1
    ...
    'or3_count', COALESCE((
      SELECT or3_count FROM public.outreach_daily_stats
      WHERE organization_id = org_id
      AND (campaign_id IS NULL OR campaign_id = get_outreach_daily_stats.campaign_id)
      AND date_et = CURRENT_DATE AT TIME ZONE tz
      ORDER BY created_at DESC LIMIT 1
...
-- 7. Create function to increment daily stats
CREATE OR REPLACE FUNCTION increment_outreach_daily_stat(org_id uuid, campaign_id uuid, stage text)
RETURNS void AS $$
DECLARE
  tz text := 'America/New_York';
  current_date_et date := CURRENT_DATE AT TIME ZONE tz;
BEGIN
  INSERT INTO public.outreach_daily_stats (organization_id, campaign_id, date_et, or1_count, or2_count, or3_count)
  VALUES (org_id, campaign_id, current_date_et,
    CASE WHEN stage = 'OR1' THEN 1 ELSE 0 END,
    CASE WHEN stage = 'OR2' THEN 1 ELSE 0 END,
    CASE WHEN stage = 'OR3' THEN 1 ELSE 0 END
  )
  ON CONFLICT (organization_id, campaign_id, date_et)
  DO UPDATE SET
    or1_count = CASE WHEN stage = 'OR1' THEN outreach_daily_stats.or1_count + 1 ELSE outreach_daily_stats.or1_count END,
    or2_count = CASE WHEN stage = 'OR2' THEN outreach_daily_stats.or2_count + 1 ELSE outreach_daily_stats.or2_count END,
    or3_count = CASE WHEN stage = 'OR3' THEN outreach_daily_stats.or3_count + 1 ELSE outreach_daily_stats.or3_count END,
    updated_at = now();

  -- Update total_sent
  UPDATE public.outreach_daily_stats
  SET total_sent = or1_count + or2_count + or3_count, updated_at = now()
  WHERE organization_id = org_id AND campaign_id = campaign_id AND date_et = current_date_et;
END;
$$ LANGUAGE plpgsql;

-- 8. Create function to get available leads for outreach
...
-- 11. Create default campaign for existing organizations
...
-- 12. Update existing leads to have proper outreach stage
UPDATE public.leads
